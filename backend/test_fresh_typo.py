"""
Fresh test for typo matching - clears everything first
"""
import requests
import time
import uuid
...
# Use unique tenant each time
UNIQUE_TENANT = f"typo-test-{uuid.uuid4().hex[:8]}"
API_KEY = f"Bearer sc-{UNIQUE_TENANT}-test"

def test():
    """Test with completely fresh tenant."""
    headers = {
    ...
    print("=" * 60)
    print("Fresh Typo Matching Test")
    print("=" * 60)
    print(f"Using unique tenant: {UNIQUE_TENANT}")
    print(f"API Key: {API_KEY}")
    
    # Query 1: "what is comptr" (first time - should be miss)
    print("\n1. Query: 'what is comptr' (first time)")
    print("   Expected: MISS (nothing in cache)")
    ...
    print("   Expected: SEMANTIC HIT (similarity ~0.80)")
    ...
    if response2.status_code == 200:
        ...
        if hit2 == 'semantic':
            ...
            print(f"\n   [WARNING] Got exact match (unexpected)")
            print(f"   This suggests normalization issue")
        else:
            print(f"\n   [ISSUE] Expected semantic match but got {hit2}")
            print(f"   Similarity {sim2:.4f} < Threshold {threshold2}")
            if sim2 == 0:
                print(f"   [CRITICAL] Similarity is 0.0 - semantic search not working!")
    else:
